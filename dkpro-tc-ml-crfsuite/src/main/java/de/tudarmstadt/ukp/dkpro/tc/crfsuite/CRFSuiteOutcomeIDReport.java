...
public class CRFSuiteOutcomeIDReport extends ReportBase {
	...
	public void execute() throws Exception {
		...
		Properties props = generateProperties(mapping, labelGoldVsActual,
				testData);
	...
	private HashMap<String, Integer> createMappingLabel2Number(
			List<String> aLabelGoldVsActual) {
	...
	protected static Properties generateProperties(
			HashMap<String, Integer> aMapping, List<String> predictions,
			List<String> testFeatures) throws Exception {
	...
	private static String extractTCId(String line) {
		...
		String id = line.substring(begin + ID_CONSTANT_VALUE.length(),
				line.length());
