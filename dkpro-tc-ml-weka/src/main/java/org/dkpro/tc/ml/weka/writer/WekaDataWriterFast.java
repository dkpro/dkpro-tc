 ...
 */
public class WekaDataWriterFast
    implements DataWriter, Constants
{
    ...
    private BufferedWriter reOpenArffWriter() throws Exception
    {
        return new BufferedWriter(new OutputStreamWriter(
                new FileOutputStream(arffTarget, true), "utf-8"));
    }
    ...
    private String initArffOutput() throws IOException
    {
        ...
        //build header
        headerFile.append(
                "@relation dkpro-tc-generated" + System.lineSeparator() + System.lineSeparator());
        for(int i=0; i < lines.size(); i++){
            String[] split = lines.get(i).split("\t");
            
            FeatureType ft = FeatureType.valueOf(split[1]);
            if(ft == FeatureType.NUM || ft == FeatureType.NUM_FLOATING_POINT || ft == FeatureType.NUM_INTEGER){
                headerFile.append("@attribute " + split[0] + " numeric" + System.lineSeparator());
                ...
                Class<Enum> c=null;
                try {
                    headerFile.append("@attribute "+split[0]+" {");
                ...
                headerFile.append("@attribute " + split[0] + " string" + System.lineSeparator());
                name2featureType.put(split[0], STRING);
            }
            
        }

        headerFile.append("@attribute outcome {");
        for(int j=0; j < outcomes.length; j++){
            headerFile.append(outcomes[j]);
            if(j+1 < outcomes.length){
                headerFile.append(",");
            }
        }
        headerFile.append("}" + System.lineSeparator()+ System.lineSeparator());
