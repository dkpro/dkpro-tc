    ...
    implements DocumentFeatureExtractor
{
        ...
        throws TextClassificationException
    {
        ...
        featList.add(new SimpleFeature(FN_NR_OF_CHARS, nrOfChars));

        double charPerSentence = 0.0;
        if (nrOfSentences > 0) {
            charPerSentence = (double) nrOfChars / nrOfSentences;
        }
        featList.add(new SimpleFeature(FN_NR_OF_CHARS_PER_SENTENCE, charPerSentence));

        double charPerToken = 0.0;
        if (nrOfTokens > 0) {
            charPerToken = (double) nrOfChars / nrOfTokens;
        }
        featList.add(new SimpleFeature(FN_NR_OF_CHARS_PER_TOKEN, charPerToken));
