 ...
 * 
 * This uses the Semantic Text Similarity (STS) from the SemEval 2012 task. It
 * computes text similarity features between document pairs and <br>
 * then learns a regression model that predicts similarity of unseen document
 * pairs.
 */
public class WekaPairRegressionDemo implements Constants {
	...
	public static ParameterSpace getParameterSpace() throws ResourceInitializationException {
		...
		CollectionReaderDescription readerTrain = CollectionReaderFactory.createReaderDescription(STSReader.class,
				STSReader.PARAM_INPUT_FILE, inputFileTrain, STSReader.PARAM_GOLD_FILE, goldFileTrain);
		dimReaders.put(DIM_READER_TRAIN, readerTrain);

		CollectionReaderDescription readerTest = CollectionReaderFactory.createReaderDescription(STSReader.class,
		...
		ParameterSpace pSpace = new ParameterSpace(Dimension.createBundle(Constants.DIM_READER_TRAIN, dimReaders),
				Dimension.create(Constants.DIM_FEATURE_MODE, Constants.FM_PAIR),
				Dimension.create(Constants.DIM_LEARNING_MODE, Constants.LM_REGRESSION), dimFeatureSets,
