 ...
 * 
 */
public class CrossValidationReport extends TcBatchReportBase implements Constants {
	...
	public void execute() throws Exception {
		...
		if (learningMode.equals(LM_SINGLE_LABEL)) {
			writeOverallResults(learningMode, store, idPool, FILE_COMBINED_BASELINE_MAJORITY_OUTCOME_KEY, true,
					"majorityBaseline_");
			writeOverallResults(learningMode, store, idPool, FILE_COMBINED_BASELINE_RANDOM_OUTCOME_KEY, true,
					"randomBaseline_");
			writeResultsPerFold(learningMode, store, idPool, BASELINE_MAJORITIY_ID_OUTCOME_KEY, true,
					"majorityBaseline_");
			writeResultsPerFold(learningMode, store, idPool, BASELINE_RANDOM_ID_OUTCOME_KEY, true, "randomBaseline_");
		}

	}

	private void writeOverallResults(String learningMode, StorageService store, Set<String> idPool,
			String outcomeFileName, boolean isBaseline, String prefix) throws Exception {
		...
		for (String id : idPool) {
			...
			if (isSingleLabelMode(learningMode) && !isBaseline) {
				...
				String matrixName = getMLSetup(id) + registerGetMapping(id) + "_" + FILE_CONFUSION_MATRIX;
	...
	private void writeResultsPerFold(String learningMode, StorageService store, Set<String> idPool,
		...
		if (isBaseline) {
			...
			targetFile = getContext().getFile(prefix + EVAL_FILE_NAME_PER_FOLD + FILE_ENDING, AccessMode.READWRITE);
		}
